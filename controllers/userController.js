...
export async function getMyTwoFactorStatus(req, res) {
  try {
    const id = req.user?._id || req.user?.id;
    ...
    const user = await User.findById(id).select("+mfaEnabled");
    ...
    return sendServerError(res, "Failed to fetch 2FA status.");
  }
}

export async function setupMyTwoFactor(req, res) {
  try {
    const id = req.user?._id || req.user?.id;
    ...
    const secret = speakeasy.generateSecret({
      name: `KnockoutCodes (${user.email})`,
      issuer: "KnockoutCodes",
      length: 20,
    });
    ...
    return sendServerError(res, "Failed to start 2FA setup.");
  }
}

export async function verifyMyTwoFactor(req, res) {
  try {
    const id = req.user?._id || req.user?.id;
    ...
    return sendServerError(res, "Failed to verify 2FA.");
  }
}

export async function disableMyTwoFactor(req, res) {
  try {
    const id = req.user?._id || req.user?.id;
    ...
    const user = await User.findById(id).select(
      "+password +mfaEnabled +mfaSecret +mfaBackupCodes",
    );
    ...
    return sendServerError(res, "Failed to disable 2FA.");
  }
}
